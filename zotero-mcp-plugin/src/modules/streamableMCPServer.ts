  ...
  switch (toolName) {
    ...
    case "extract_pdf_metadata":
      return {
        ...
        dataStructure: {
          type: "pdf_metadata",
          format: "Structured PDF document properties and metadata fields",
          fields:
            "Document information (title, author, subject, keywords) and technical metadata (creator, producer, dates, page count)",
        },
        interpretation: {
          purpose:
            "Direct extraction of document metadata embedded in PDF files",
          content:
            "Information embedded by document creators, publishers, or PDF generation tools",
          reliability:
            "Metadata quality varies - may be incomplete, outdated, or inaccurate depending on source PDF",
        },
        usage: [
          "Use for initial assessment of PDF document properties before import or analysis",
          "Quickly check if PDF has embedded title/author information for citation purposes",
          "May not match actual document content - always verify against document text for accuracy",
          "Use in combination with content analysis tools for complete document understanding",
          "If metadata is insufficient, consider using upload_pdf_and_create_item for Zotero Web API enrichment",
        ],
      };
 ...
 * that runs directly within the Zotero ext. AI clients can connect using
...
export class StreamableMCPServer {
  ...
  private handleToolsList(request: MCPRequest): MCPResponse {
    const tools = [
      ...
      },
      {
        ...
        description:
          "Search annotations and notes with intelligent ranking and content management",
        inputSchema: {
          type: "object",
          properties: {
      ...
      },
      {
        ...
        description:
          "Get annotations and notes with intelligent content management (PDF annotations, highlights, notes)",
        inputSchema: {
          type: "object",
          properties: {
            itemKey: {
      ...
      },
      {
        ...
        description:
          "Generate a citation for a specific item in various CSL styles (APA, Chicago, Harvard, IEEE, MLA, Nature, Vancouver, BibTeX)",
        inputSchema: {
          type: "object",
          properties: {
            ...
            format: {
              ...
              description:
                "Output format: html (with formatting), text (plain text), bibtex (BibTeX entry)",
            },
            itemKeys: {
              type: "array",
              items: { type: "string" },
              description:
                "Multiple item keys to generate citations for (batch mode)",
            },
          },
          required: ["itemKey"],
        },
      },
      {
        ...
        description:
          "Upload a PDF file, extract metadata using Zotero web service and PDF properties, and create a new Zotero item with the PDF attached",
        inputSchema: {
          type: "object",
          properties: {
            pdfPath: {
            ...
            itemType: {
              type: "string",
              description:
                "Optional item type override (default: auto-detect from metadata)",
            },
            useWebService: {
              type: "boolean",
              default: true,
              description:
                "Use Zotero web service to retrieve metadata (default: true)",
            },
            extractPDFProperties: {
              type: "boolean",
              default: true,
              description:
                "Extract metadata from PDF document properties (default: true)",
            },
          },
          required: ["pdfPath"],
        },
      },
      {
        ...
        description:
          "Enrich an existing Zotero item by extracting metadata from its PDF attachment using Zotero web service and PDF properties",
        inputSchema: {
          type: "object",
          properties: {
            itemKey: {
            ...
            attachmentKey: {
              type: "string",
              description:
                "Specific PDF attachment key (default: use first PDF attachment)",
            },
            overwriteExisting: {
              type: "boolean",
              default: false,
              description:
                "Overwrite existing field values (default: false, only fill empty fields)",
            },
            useWebService: {
              type: "boolean",
              default: true,
            ...
            fieldsToUpdate: {
              ...
              items: { type: "string" },
              description:
                "Limit updates to specific fields (default: all fields)",
            },
          },
          required: ["itemKey"],
        },
      },
      {
        ...
        description:
          "Extract metadata directly from a PDF file including title, author, subject, keywords, creation date, and document properties",
        inputSchema: {
          type: "object",
          properties: {
            pdfPath: {
              type: "string",
              description:
                "Absolute path to the PDF file to extract metadata from",
            },
          },
          required: ["pdfPath"],
        },
    ...
    try {
      ...
      switch (name) {
        ...
        case "extract_pdf_metadata":
          ...
          result = await this.callExtractPdfMetadata(args);
          break;
  ...
  private async callGetItemCitation(args: any): Promise<any> {
    ...
    if (itemKeys && Array.isArray(itemKeys) && itemKeys.length > 0) {
      ...
      const effectiveFormat =
        format === "bibtex" ? "bibtex" : format === "html" ? "html" : "text";

      const result = await generateMultipleCitations(
        items,
        effectiveStyle,
        effectiveFormat,
      );
  ...
  private async callUploadPdfAndCreateItem(args: any): Promise<any> {
    ...
    try {
      ztoolkit.log("[StreamableMCP] upload_pdf_and_create_item:", {
        pdfPath,
        collectionKey,
      });
      ...
      if (useWebService) {
        ...
        try {
          ...
          ztoolkit.log(
            "[StreamableMCP] Web metadata retrieval failed:",
            error,
            "error",
          );
        } finally {
      ...
      if (extractPDFProperties) {
        ...
        try {
          ...
          ztoolkit.log(
            "[StreamableMCP] PDF metadata extraction failed:",
            error,
            "error",
          );
        } finally {
      ...
      ztoolkit.log(
        "[StreamableMCP] upload_pdf_and_create_item failed:",
        error,
        "error",
      );
      throw error;
  ...
  private async callEnrichItemFromPdf(args: any): Promise<any> {
    ...
    try {
      ztoolkit.log("[StreamableMCP] enrich_item_from_pdf:", {
        itemKey,
        attachmentKey,
      });
      ...
      if (fieldsToUpdate && Array.isArray(fieldsToUpdate) && webMetadata) {
        ...
        for (const field of fieldsToUpdate) {
          if (webMetadata[field as keyof typeof webMetadata]) {
            filteredMetadata[field] =
              webMetadata[field as keyof typeof webMetadata];
          }
        }
      ...
      ztoolkit.log(
        "[StreamableMCP] enrich_item_from_pdf failed:",
        error,
        "error",
      );
      throw error;
  ...
  private async callExtractPdfMetadata(args: any): Promise<any> {
    const { pdfPath } = args;

    try {
      ztoolkit.log("[StreamableMCP] extract_pdf_metadata:", { pdfPath });
      ...
      try {
        metadata = await extractor.extractMetadata(pdfPath);
        ztoolkit.log("[StreamableMCP] PDF metadata extracted:", metadata);
      } catch (error) {
        ztoolkit.log(
          "[StreamableMCP] PDF metadata extraction failed:",
          error,
      ...
      // Build response with comprehensive metadata information
      const responseData = {
        pdfPath,
        metadata: {
        ...
        summary: {
          ...
          keywordsCount: metadata.keywords ? metadata.keywords.length : 0,
          totalFields: Object.keys(metadata).length,
        },
      };

      return applyGlobalAIInstructions(responseData, "extract_pdf_metadata");
    } catch (error) {
      ztoolkit.log(
        "[StreamableMCP] extract_pdf_metadata failed:",
        error,
        "error",
      );
      throw error;
